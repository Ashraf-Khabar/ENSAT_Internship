...
const Applications = () => {
  ...
  const getApplications = () => {
    axios
      .get("http://localhost:5000/Applications", { params: { id: id } })
      .then((response) => {
    ...
    <div className="flex ">
      <div className="scrollable-container flex w-2/5 md:w-1/4 h-screen bg-white">
        <div className="mx-auto py-10">
          <ul>
            {/* Home is where you'll find all the offers */}
        ...
        <div className="overflow-y: scroll items-center justify-center ml-10 mr-10 mt-5  bg-white rounded-lg shadow dark:bg-gray-800">
          <ul className="flex flex-col mb-2 divide-y divide">
            {applicationList.map((application, key) => (
              ...
              <Link
                className="flex flex-row   hover:bg-indigo-200"
                to={{ pathname: "/dashboard/offer/" + application.OfferId }}
              >
                <div className="flex items-center flex-1 p-4 cursor-pointer select-none">
                  ...
                  <div className="flex-1 pl-1 mr-16">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-200">
                      {application.Offer.Employer.denomination}
                    </div>
                  </div>
                  <div className="text-xs text-gray-600 dark:text-gray-200">
                     {application.rang ? "Rank :"+ application.rang : "State : In Process"}
                  </div>
                </div>
              </Link>
            ))}
